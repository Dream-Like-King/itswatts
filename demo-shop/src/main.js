...
const legalNotices = {
  privacy: { label: 'PRIVACY', title: 'Privacy notice', content: `<p><strong>Last updated: August 1, 2026.</strong> The Demo Lab is a fictional QA practice environment. Please do not enter real passwords, banking details, personal information, or confidential work information.</p><h3>Practice data only</h3><p>Customer profiles, accounts, employees, claims, orders, and balances shown in this lab are fictional. Sign-in and profile changes happen only in your current browser session; they are not real accounts or services.</p><h3>Related site services</h3><p>The main It’s Watts site may use services such as Google Analytics and Ask Watt. Its privacy notice is available at <a href="https://itswatts.com">itswatts.com</a>.</p>` },
  terms: { label: 'TERMS', title: 'Terms of use', content: `<p><strong>Last updated: August 1, 2026.</strong> You may use the Demo Lab for personal learning, QA practice, exploratory testing, and automation practice.</p><h3>Use it safely</h3><p>Do not submit real or sensitive information, attempt to disrupt the service, or misuse the environment. This lab is educational only and does not provide banking, insurance, HR, employment, purchasing, or security services.</p><p class="lab-legal-note">These starter terms should be reviewed by a qualified legal professional before relying on them as formal legal terms.</p>` },
  accessibility: { label: 'ACCESSIBILITY', title: 'Accessibility statement', content: `<p>It’s Watts aims to make its learning tools and Demo Lab usable by as many people as possible through clear structure, keyboard-friendly interactions, readable contrast, and responsive layouts.</p><h3>Send feedback</h3><p>If something is difficult to use or unavailable with your assistive technology, tell us which page and device you were using.</p><p><a href="mailto:contact@itswatts.com?subject=Demo%20Lab%20accessibility%20feedback">Send accessibility feedback →</a></p>` },
  notice: { label: 'DEMO LAB', title: 'Fictional practice notice', content: `<p>This is a QA training environment—not a real store, financial institution, claims provider, or HR system.</p><h3>No real transactions or records</h3><p>All names, accounts, balances, products, claims, employee profiles, and outcomes are fictional. Demo actions may change or reset without notice. Do not use this environment for real purchases, payments, claims, employment decisions, or financial services.</p>` },
}

function openLegalNotice(name) {
  const notice = legalNotices[name]
  if (!notice) return
  byId('legal-dialog-label').textContent = notice.label
  byId('legal-dialog-title').textContent = notice.title
  byId('legal-dialog-content').innerHTML = notice.content
  byId('legal-dialog').hidden = false
  byId('legal-dialog-close').focus()
}

function closeLegalNotice() { byId('legal-dialog').hidden = true }
...
document.querySelectorAll('[data-legal-trigger]').forEach((button) => button.addEventListener('click', () => openLegalNotice(button.dataset.legalTrigger)))
byId('legal-dialog-close').addEventListener('click', closeLegalNotice)
byId('legal-dialog').addEventListener('click', (event) => { if (event.target === event.currentTarget) closeLegalNotice() })
window.addEventListener('keydown', (event) => { if (event.key === 'Escape') closeLegalNotice() })
