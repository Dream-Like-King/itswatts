...
function ResourceCard({ resource, onOpenChat, onOpenToolkits, onOpenLearn, onOpenWeekly }: { resource: Resource; onOpenChat: () => void; onOpenToolkits: () => void; onOpenLearn: () => void; onOpenWeekly: () => void }) {
  return <article>
    <div className="resource-meta"><span>{resource.category}</span><i>{resource.type}</i></div>
    <h3>{resource.title}</h3>
    <p>{resource.description}</p>
    {resource.title === 'Ask Watt' ? <button type="button" className="resource-open" onClick={onOpenChat}>Open resource <b aria-hidden="true">↗</b></button> : resource.href === '#tools' ? <button type="button" className="resource-open" onClick={onOpenToolkits}>Open resource <b aria-hidden="true">↗</b></button> : resource.href === '#learn' ? <button type="button" className="resource-open" onClick={onOpenLearn}>Open resource <b aria-hidden="true">↗</b></button> : resource.href === '#weekly' ? <button type="button" className="resource-open" onClick={onOpenWeekly}>Open resource <b aria-hidden="true">↗</b></button> : <a href={resource.href} download={resource.download}>{resource.download ? 'Download resource' : 'Open resource'} <b aria-hidden="true">↗</b></a>}
  </article>
}

export function ResourceLibrary({ onOpenChat, onOpenToolkits, onOpenLearn, onOpenWeekly }: { onOpenChat: () => void; onOpenToolkits: () => void; onOpenLearn: () => void; onOpenWeekly: () => void }) {
  ...
  return <section className="resource-library section" id="resources">
    ...
    {isOpen && <div className="resource-overlay" role="presentation" onMouseDown={() => setIsOpen(false)}>
      ...
      <div className="resource-grid resource-full">{visibleResources.map((resource) => <ResourceCard key={resource.title} resource={resource} onOpenChat={onOpenChat} onOpenToolkits={onOpenToolkits} onOpenLearn={onOpenLearn} onOpenWeekly={onOpenWeekly} />)}</div>
      </article>
    </div>}
  </section>
}
