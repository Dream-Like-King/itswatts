...
export function PracticeHub({ onClose, onOpenTools, onOpenLearn, theme, onToggleTheme }: PracticeHubProps) {
  ...
  return <main className="practice-hub" id="top" tabIndex={-1}>
    ...
    <section className="practice-content" aria-live="polite">
      <DeepDiveArticle label="PRACTICE LAB · FULL ARTICLE" title="Turn a QA exercise into a real skill." intro="Practice becomes useful when you make a decision, observe the evidence, explain your reasoning, and reflect on what you would do differently in a real product." sections={[["Before you begin", "Write down the user goal, the product risk, and what evidence would change your mind. This makes a scenario more than a guessing game."], ["While you test", "Explore one variable at a time. Record what you expected, what happened, the data or account you used, and any clue that would help another person reproduce it."], ["After you answer", "Review why the strongest choice protects users or delivery. Then translate the lesson into one check, question, or test note you could take into a real sprint."]]} />
      ...
      {activeTab === 'scenarios' && <div className="scenario-panel"><div><p className="eyebrow">PRACTICE SCENARIO · {String(scenarioIndex + 1).padStart(2, '0')}/{String(scenarios.length).padStart(2, '0')}</p><h2>{scenario.title}</h2><p>{scenario.situation}</p></div><div className="scenario-choices">{scenario.choices.map(([choice, isBest, feedback], index) => <button key={choice} type="button" className={selectedChoice === index ? (isBest ? 'correct' : 'incorrect') : ''} onClick={() => { setSelectedChoice(index); setScenarioResults((current) => ({ ...current, [scenarioIndex]: isBest })) }}><span>{String.fromCharCode(65 + index)}</span><b>{choice}</b>{selectedChoice === index && <small>{feedback}</small>}</button>)}</div><div className="scenario-controls"><button type="button" disabled={scenarioIndex === 0} onClick={() => { setScenarioIndex((index) => index - 1); setSelectedChoice(null) }}>← Previous</button><button type="button" disabled={scenarioIndex === scenarios.length - 1} onClick={() => { setScenarioIndex((index) => index + 1); setSelectedChoice(null) }}>Next scenario →</button></div>{scenarioIndex === scenarios.length - 1 && selectedChoice !== null && <aside className="practice-recap"><p className="eyebrow">PRACTICE RECAP</p><h3>{Object.values(scenarioResults).filter(Boolean).length} of {scenarios.length} strong QA decisions</h3><p>{Object.values(scenarioResults).filter(Boolean).length >= 4 ? 'You are recognizing risk-focused coverage well. Try Bug Hunt next to apply that thinking in a simulated product.' : 'Review the feedback, then try Bug Hunt to practice observing evidence before choosing a report.'}</p><button type="button" onClick={() => setActiveTab('bug-hunt')}>Open Bug Hunt <span>↗</span></button></aside>}</div>}
