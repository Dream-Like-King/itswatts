...
export function BugHunt() {
  ...
  const [selectedBug, setSelectedBug] = useState<BugId>('cart-total')
  ...
  const submitReport = () => {
    setReported((current) => current.includes(selectedBug) ? current : [...current, selectedBug])
    setReportOpen(false)
  }

  const reset = () => {
    setQuantity(1); setCode(''); setPromoState('idle'); setExpressShipping(false); setEmail('customer@example.com'); setOrderCount(0); setReportOpen(false); setReported([]); setShowReview(false)
  }
  ...
  return <div className="bug-hunt-panel">
    ...
    {reportOpen && <div className="bug-modal-backdrop" role="presentation" onMouseDown={() => setReportOpen(false)}><section className="bug-report-modal" role="dialog" aria-modal="true" aria-labelledby="bug-report-title" onMouseDown={(event) => event.stopPropagation()}><button className="bug-modal-close" type="button" aria-label="Close report form" onClick={() => setReportOpen(false)}>×</button><p className="eyebrow">NEW BUG REPORT</p><h3 id="bug-report-title">Capture the behavior.</h3><p>Choose the issue that best matches what you observed. In a real report, you would also include environment, evidence, and reproducible steps.</p><label htmlFor="bug-selection">OBSERVED ISSUE</label><select id="bug-selection" value={selectedBug} onChange={(event) => setSelectedBug(event.target.value as BugId)}>{bugs.map((bug) => <option key={bug.id} value={bug.id}>{bug.title}</option>)}</select><div className="bug-report-preview"><span>EXPECTED</span><p>{bugs.find((bug) => bug.id === selectedBug)?.expected}</p><span>ACTUAL</span><p>{bugs.find((bug) => bug.id === selectedBug)?.actual}</p></div><button type="button" className="button primary" onClick={submitReport}>{reported.includes(selectedBug) ? 'Already reported' : 'Submit bug report'} <span>↗</span></button></section></div>}

    {showReview && <div className="bug-modal-backdrop" role="presentation" onMouseDown={() => setShowReview(false)}><section className="bug-report-modal bug-review-modal" role="dialog" aria-modal="true" aria-labelledby="bug-review-title" onMouseDown={(event) => event.stopPropagation()}><button className="bug-modal-close" type="button" aria-label="Close review" onClick={() => setShowReview(false)}>×</button><p className="eyebrow">BUG HUNT REVIEW · {reportedCount}/5</p><h3 id="bug-review-title">{reportedCount === 5 ? 'Great investigation.' : 'Keep exploring.'}</h3><p>Good QA is not only about finding an issue—it is about describing its impact and making it easy for the team to act.</p><div className="bug-review-list">{bugs.map((bug) => <article key={bug.id} className={reported.includes(bug.id) ? 'reported' : ''}><span>{reported.includes(bug.id) ? 'FOUND' : 'MISSED'}</span><h4>{bug.title}</h4><p><b>Expected:</b> {bug.expected}</p><p><b>Actual:</b> {bug.actual}</p></article>)}</div><button type="button" className="button primary" onClick={() => setShowReview(false)}>Continue practicing ↗</button></section></div>}
  </div>
}
