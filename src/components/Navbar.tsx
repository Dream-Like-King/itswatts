...
type NavbarProps = { onOpenChat: () => void; onOpenLearn: () => void; onOpenCareer: () => void; onOpenStory: () => void; onOpenToolkits: () => void; onOpenResources: () => void; onOpenSoftware: () => void }

export function Navbar({ onOpenChat, onOpenLearn, onOpenCareer, onOpenStory, onOpenToolkits, onOpenResources, onOpenSoftware }: NavbarProps) {
  const [open, setOpen] = useState(false)
  return <header className="nav-wrap">
    <nav className="nav" aria-label="Main navigation">
      <Logo />
      <div id="main-navigation-links" className={open ? 'nav-links open' : 'nav-links'}>
        {links.map(([label, target]) => target === 'learn' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenLearn() }}>{label}</button> : target === 'career-paths' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenCareer() }}>{label}</button> : target === 'tools' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenToolkits() }}>{label}</button> : target === 'resources' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenResources() }}>{label}</button> : target === 'focus-tools' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenSoftware() }}>{label}</button> : target === 'my-story' ? <button className="nav-link-button" key={target} type="button" onClick={() => { setOpen(false); onOpenStory() }}>{label}</button> : <a key={target} href={`#${target}`} onClick={() => setOpen(false)}>{label}</a>)}
      </div>
