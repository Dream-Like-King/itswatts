...
function App() {
  ...
  const openLearn = (target = 'top') => { setLearnTarget(target); window.history.pushState(null, '', '#learn-hub'); setIsSoftwareOpen(false); setIsResourcesOpen(false); setIsToolkitsOpen(false); setIsPracticeOpen(false); setIsStoryOpen(false); setIsLearnOpen(true) }
  const openStory = () => { window.history.pushState(null, '', '#my-story'); setIsSoftwareOpen(false); setIsResourcesOpen(false); setIsToolkitsOpen(false); setIsPracticeOpen(false); setIsLearnOpen(false); setIsStoryOpen(true); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })) }
  const openPractice = () => { window.history.pushState(null, '', '#practice-lab'); setIsSoftwareOpen(false); setIsResourcesOpen(false); setIsToolkitsOpen(false); setIsStoryOpen(false); setIsLearnOpen(false); setIsPracticeOpen(true); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })) }
  const openToolkits = () => { window.history.pushState(null, '', '#qa-toolkits'); setIsSoftwareOpen(false); setIsResourcesOpen(false); setIsPracticeOpen(false); setIsStoryOpen(false); setIsLearnOpen(false); setIsToolkitsOpen(true); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })) }
  const openResources = () => { window.history.pushState(null, '', '#resources-hub'); setIsSoftwareOpen(false); setIsToolkitsOpen(false); setIsPracticeOpen(false); setIsStoryOpen(false); setIsLearnOpen(false); setIsResourcesOpen(true); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })) }
  const openSoftware = () => { window.history.pushState(null, '', '#qa-software'); setIsResourcesOpen(false); setIsToolkitsOpen(false); setIsPracticeOpen(false); setIsStoryOpen(false); setIsLearnOpen(false); setIsSoftwareOpen(true); window.requestAnimationFrame(() => window.scrollTo({ top: 0, behavior: 'smooth' })) }
  const closeFocusedView = () => { window.history.pushState(null, '', '#top'); setIsLearnOpen(false); setIsStoryOpen(false); setIsPracticeOpen(false); setIsToolkitsOpen(false); setIsResourcesOpen(false); setIsSoftwareOpen(false) }
  useEffect(() => {
    const syncFocusedView = () => { setIsLearnOpen(window.location.hash === '#learn-hub'); setIsStoryOpen(window.location.hash === '#my-story'); setIsPracticeOpen(window.location.hash === '#practice-lab'); setIsToolkitsOpen(window.location.hash === '#qa-toolkits'); setIsResourcesOpen(window.location.hash === '#resources-hub'); setIsSoftwareOpen(window.location.hash === '#qa-software') }
  ...
  return <>
    ...
    <section className="hero education-hero">
      ...
      <div className="hero-content"><p className="eyebrow hero-eyebrow"><span></span> AUTOMATION · AI · QUALITY ENGINEERING</p><h1>Practical QA education<br />for the <em>AI era.</em></h1><p className="hero-copy">Learn the testing habits, automation patterns, and AI workflows that help you build software people can trust.</p><div className="hero-actions"><a className="button primary" href="#learn">Start learning <span>↓</span></a><button type="button" className="button quiet" onClick={openToolkits}>Open QA Toolkits <span>↗</span></button></div></div>
    ...
    <MobileDock onOpenChat={() => setIsChatOpen(true)} onOpenLearn={openLearn} onOpenCareer={() => openLearn('career-paths')} onOpenStory={openStory} onOpenPractice={openPractice} onOpenToolkits={openToolkits} onOpenResources={openResources} onOpenSoftware={openSoftware} onNavigateHome={isLearnOpen || isStoryOpen || isPracticeOpen || isToolkitsOpen || isResourcesOpen || isSoftwareOpen ? navigateHome : undefined} focusedView={isStoryOpen ? 'story' : isPracticeOpen ? 'practice' : isToolkitsOpen ? 'toolkits' : isResourcesOpen ? 'resources' : isSoftwareOpen ? 'software' : isLearnOpen ? 'learn' : null} />
