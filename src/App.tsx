...
function App() {
  ...
  const closeFocusedView = () => { window.history.pushState(null, '', '#top'); setIsLearnOpen(false); setIsStoryOpen(false); setIsPracticeOpen(false); setIsToolkitsOpen(false); setIsResourcesOpen(false); setIsSoftwareOpen(false); setIsCareerOpen(false); setIsApiGuideOpen(false); setIsRagTrainingOpen(false); setIsTestDesignOpen(false) }
  useEffect(() => {
    const syncFocusedView = () => { setIsLearnOpen(window.location.hash === '#learn-hub'); setIsStoryOpen(window.location.hash === '#my-story'); setIsPracticeOpen(window.location.hash === '#practice-lab'); setIsToolkitsOpen(window.location.hash === '#qa-toolkits'); setIsResourcesOpen(window.location.hash === '#resources-hub'); setIsSoftwareOpen(window.location.hash === '#qa-software'); setIsCareerOpen(window.location.hash === '#career-paths'); setIsApiGuideOpen(window.location.hash === '#api-testing-guide'); setIsRagTrainingOpen(window.location.hash === '#rag-training'); setIsTestDesignOpen(window.location.hash === '#test-design') }
  ...
  return <>
    ...
    <MobileDock onOpenChat={() => setIsChatOpen(true)} onOpenLearn={openLearn} onOpenCareer={openCareer} onOpenStory={openStory} onOpenPractice={openPractice} onOpenToolkits={openToolkits} onOpenResources={openResources} onOpenSoftware={openSoftware} onNavigateHome={isLearnOpen || isStoryOpen || isPracticeOpen || isToolkitsOpen || isResourcesOpen || isSoftwareOpen || isCareerOpen || isApiGuideOpen || isRagTrainingOpen || isTestDesignOpen ? navigateHome : undefined} focusedView={isStoryOpen ? 'story' : isPracticeOpen ? 'practice' : isToolkitsOpen ? 'toolkits' : isResourcesOpen ? 'resources' : isSoftwareOpen ? 'software' : isCareerOpen ? 'career' : isApiGuideOpen || isRagTrainingOpen || isTestDesignOpen || isLearnOpen ? 'learn' : null} />
